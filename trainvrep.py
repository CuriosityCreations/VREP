...
def sarsa1_minibatch(minibatch, model, sarsa0P):
  ...
  for memory in minibatch:     
    # Get stored values.
    old_state_m, action_m, reward_m, new_state_m, new_action = memory
    ...
    if reward_m != sarsa0P[0]:  # non-terminal state
      update = (reward_m + (sarsa0P[1] * updateQ))
    else:  # terminal state
      update = reward_m
